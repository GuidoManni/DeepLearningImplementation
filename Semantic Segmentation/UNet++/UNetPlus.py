...
class ExpandingPath(nn.Module):
    def __init__(self, n_classes):
        ...
        self.fourth_level = ConvBlock(in_channels=1024, out_channels=512, maxpool=False)  # X₄,₁

        # Third level blocks
        self.third_level = ConvBlock(in_channels=512, out_channels=256, maxpool=False)  # X₃,₁

        # Second level blocks
        self.second_level = ConvBlock(in_channels=256, out_channels=128, maxpool=False)  # X₂,₁

        # First level blocks (closest to output)
        self.first_level = ConvBlock(in_channels=128, out_channels=64, maxpool=False)  # X₁,₁
    ...
    def forward(self, output_features):
        ...
        # Level 3: Two possible paths
        # Path 1:
        x2_1 = self.up2(x3_0)
        x2_1 = self.cc(x2_0, x2_1) # skip connection x2_0 -> x2_1
        x2_1 = self.third_level(x2_1)

        # Path 2
        x2_2 = self.up2(x3_1)
        x2_2 = self.cc(x2_1, x2_2) # skip connection x2_1 -> x2_2
        x2_2 = self.third_level(x2_2)

        # Level 2: 3 possible paths
        # Path 1:
        x1_1 = self.up3(x2_0)
        x1_1 = self.cc(x1_0, x1_1) # skip connection x1_0 -> x1_1
        x1_1 = self.second_level(x1_1)

        # Path 2:
        x1_2 = self.up3(x2_1)
        x1_2 = self.cc(x1_1, x1_2)
        x1_2 = self.second_level(x1_2)

        # Path 3:
        x1_3 = self.up3(x2_2)
        x1_3 = self.cc(x1_2, x1_3)
        x1_3 = self.second_level(x1_3)
        ...
        x0_1 = self.first_level(x0_1)

        # Path 2:
        x0_2 = self.up4(x1_1)
        x0_2 = self.cc(x0_1, x0_2)
        x0_2 = self.first_level(x0_2)

        # Path 3:
        x0_3 = self.up4(x1_2)
        x0_3 = self.cc(x0_2, x0_3)
        x0_3 = self.first_level(x0_3)

        # Path 4:
        x0_4 = self.up4(x1_3)
        x0_4 = self.cc(x0_3, x0_4)
        x0_4 = self.first_level(x0_4)
